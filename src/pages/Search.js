...
const Search = () => {
  ...
  useEffect(() => {
    const search = async () => {
      const searched = await fetch(
        `https://api.themoviedb.org/3/search/multi?api_key=08a7337c36b62d4a8a9dfafd26b3afb6&${
          query && `query=${query}`
        }&page=${searchPage}`
      );
      ...
      if (searchedJson.results.length > 0) {
        caches
          .open('searched')
          .then((cache) =>
            cache.put('searched', new Response(JSON.stringify(searchedJson)))
          );
      }
    };
    search();
  ...
  return (
    <div className='Search'>
      ...
      {query ? (
        <VerticalList list={results} />
      ) : (
        <p className='noResults'>NO RESULTS...</p>
      )}
      {query && (
        <Paginator
          prevFunc={turnPrevPage}
          page={searchPage}
          nextFunc={turnNextPage}
        />
      )}
    </div>
  );
