        ...
        <div className="Movie">
            ...
            <main className="movieInfo">
                ...
                <div className="watchProviders">
                    ...
                    </div> : <p className="noProviders">No providers found...</p>}
                </div>
                            ...
                            {credits.map((cast) => {
                                return (
                                    <div key={cast.cast_id} className="castMember">
                                        {cast.profile_path ? <img src={imgBaseURL + cast.profile_path} alt={cast.name} /> : <div className="castMemberImgFB"></div>}
                                        <p className="castMemberName" title="NAME">{cast.name}</p>
                                        <p className="castMemberCharacter" title="CHARACTER">{cast.character}</p>
                                    </div>
                                )
                            })}
                        </div>
                ...
                <div className="recommendations">
                    ...
                    <div className="recommendationsList">
                        {recommendations.length ? recommendations.map((movie) => {
                            return (
                                <Link to={`../movie/${movie.id}`} key={movie.id} className="recommendedMovie">
                                    {movie.backdrop_path ? <img className="recommendedTN" src={imgBaseURL + movie.backdrop_path} alt="Movie Backdrop" /> : <div className="recommendedTNFallback"></div>}
                                    <div className="recommendedMovieInfo">
                                        <p className="recommendedMovieTitle">{movie.title}</p>
                                        <p className="recommendedMovieRating"><i className="fas fa-star"></i> {movie.vote_average}</p>
                                        <p className="recommendedMovieLang"><i className="fas fa-language"></i> {movie.original_language}</p>
                                    </div>
                                </Link>
                            )
